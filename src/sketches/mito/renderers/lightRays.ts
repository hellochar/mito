...
import { randomRectPerimeter } from "math/random";
import {
  Box3,
  BufferGeometry,
...
export class LightRays {
  ...
  commit(start: Vector2, end: Vector2, brightness: number) {
...
class LightRay {
  ...
  update(dt: number, owner: LightRays) {
    ...
    // const brightness = (4 * (this.timeRemaining - this.timeRemaining ** 2)) ** (1 / 8);
    const brightness = 1;
